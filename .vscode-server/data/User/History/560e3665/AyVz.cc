...
void handle_SC_Open()
{
	...
	int isSucces = (int)kernel->fileSystem->Open(fileName);
	if (!isSucces)
	{
	...
	{
		DEBUG(dbgSys, "OpenFile addr: " << isSucces << ".\n");
		kernel->machine->WriteRegister(2, 0);
	}
...
void handle_SC_Close()
{
	...
	int flag = kernel->machine->ReadRegister(4);

	// nếu mở file không thành công, hoặc là stdin, stdout
	if (flag == 0 || flag == 1)
	{
		kernel->machine->WriteRegister(2, -1);
		DEBUG(dbgSys, "Close file return -1 "
						  << ".\n");
	}
	else
	{
		DEBUG(dbgSys, "Close file return 0 "
						  << ".\n");
		OpenFile *file = (OpenFile *)flag;
		// xóa con trỏ trỏ đến file đang mở nghĩa là đóng file đó
		delete file;
		kernel->machine->WriteRegister(2, 0);
	}
...
void handle_SC_Read()
{
	...
	char *buffer = User2System(virAdd, charCount);

	if (charCount < 0 || id == 1) //độ dài < 0 hoặc stdin
	{
		kernel->machine->WriteRegister(2, -1);
	}
	...
	else
	{
		...
		kernel->machine->WriteRegister(2, charRead);
	}

	System2User(virAdd, charCount, buffer);
...
void handle_SC_Write()
{
	...
	{
		OpenFile *file = (OpenFile *)id;
		int charWrite = file->Write(buffer, charCount);

		kernel->machine->WriteRegister(2, charWrite);
	}
...
void handle_SC_Seek()
{
	...
	{
		kernel->machine->WriteRegister(2, -1);
		DEBUG(dbgSys, "Return -1.\n");
		return IncreasePC();
	}
	DEBUG(dbgSys, "Return 0.\n");
	OpenFile *file = (OpenFile *)id;
	...
	int res = file->Read(temp, pos);

	delete[] temp;
	// nếu offset cuối cùng bằng với pos thì offset đầu vào là hợp lệ
	kernel->machine->WriteRegister(2, res == pos ? pos : -1);
	return IncreasePC();
}
