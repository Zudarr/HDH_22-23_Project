...
void handle_SC_Create()
{
	...
	char *fileName = User2System(virtAddr, 256);
...
void handle_SC_Open()
{
	...
	for (int i = 0; i < 50; ++i)
	{
		if (kernel->fileSystem->tableOfFiles[i] != NULL && strcmp(kernel->fileSystem->tableOfFiles[i]->fileName, fileName) == 0)
		{
			kernel->machine->WriteRegister(2, -1);
			delete[] fileName;
			return IncreasePC();
	...
	kernel->fileSystem->tableOfFiles[emptySlot] = kernel->fileSystem->Open(fileName);
	if (kernel->fileSystem->tableOfFiles[emptySlot] == NULL)
	{
		kernel->machine->WriteRegister(2, -1);
		delete[] fileName;
		return IncreasePC();
	}
	kernel->fileSystem->tableOfFiles[emptySlot]->fileName = new char[strlen(fileName) + 1];
	strcpy(kernel->fileSystem->tableOfFiles[emptySlot]->fileName, fileName);
	kernel->machine->WriteRegister(2, emptySlot);
	delete[] fileName;
	return IncreasePC();
}
...
void handle_SC_Close()
{
	...
	if (kernel->fileSystem->tableOfFiles[fileId] == NULL)
	{
		kernel->machine->WriteRegister(2, -1);
		return IncreasePC();
	}

	delete kernel->fileSystem->tableOfFiles[fileId];
	kernel->fileSystem->tableOfFiles[fileId] = NULL;
...
void handle_SC_Read()
{
	...
	int size = kernel->machine->ReadRegister(5);   // get number of chars from register 5
	...
	if (kernel->fileSystem->tableOfFiles[id] == NULL)
	{
		kernel->machine->WriteRegister(2, -1);
		return IncreasePC();
	}

	char *buffer = User2System(virAdd, size);
	int result = kernel->fileSystem->tableOfFiles[id]->Read(buffer, size);
	kernel->machine->WriteRegister(2, result);

	System2User(virAdd, result, buffer);
...
void handle_SC_Write()
{
	...
	char *buffer = User2System(virAdd, size);
	int result = kernel->fileSystem->tableOfFiles[id]->Write(buffer, size);
	kernel->machine->WriteRegister(2, result);
...
void handle_SC_Seek()
{
	...
	if (pos == -1)
	{
		pos = kernel->fileSystem->tableOfFiles[id]->Length();
	}
	if (pos < 0)
	{
		pos = 0;
	}
	int numbytes = kernel->fileSystem->tableOfFiles[id]->Seek(pos);
	if (numbytes >= 0)
	{
...
void handle_SC_Remove()
{
	...
	char *buffer = User2System(virAdd, 256);

	bool result = kernel->fileSystem->Remove(buffer);
	delete buffer;

	kernel->machine->WriteRegister(2, result ? 0 : -1);
	return IncreasePC();
}
